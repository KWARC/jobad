...
*/
JOBAD.util.toKeyString = function(evt){
	
	//Most of this code is borrowed form https://github.com/jeresig/jquery.hotkeys/blob/master/jquery.hotkeys.js
	//which is relaeased under MIT
	...
	var special = values.specialKeys[event.keyCode]; 
	var character = event.type === "keypress" && String.fromCharCode(event.which).toLowerCase(); 
	var modif = ""; 
	var possible = [];
	...
	if (character) {
		console.log("Have character", character); 
		possible.push(modif + character);
		if(values.shiftNums[ character ]){
			possible.push(modif + values.shiftNums[ character ]); 
		}
		

		// "$" can be triggered as "Shift+4" or "Shift+$" or just "$"
		if ( modif === "shift+" ) {
			possible.push(values.shiftNums[ character ]); 
		}
	}
...
JOBAD.util.onKey = function(cb){
	JOBAD.refs.$(document).on("keydown keyup keypress", function(evt){
		cb.call(undefined, JOBAD.util.toKeyString(evt));
	}); 
}
